...
{
  /* USER CODE BEGIN 1 */
	char is_clicked=0;	//for toggle switch
	char ledMode=0;	// 0:led off 1:led on 2:led blink
  ...
  //HAL_GPIO_ReadPin 0 means button pushed
  //HAL_GPIO_WritePin 0 means light on
  while (1)
  {
	  // button clicked!! change ledMode!
	  if(is_clicked==2){
		  ledMode++;
		  ledMode=ledMode%3;
	  ...
	  if(is_clicked==1&&HAL_GPIO_ReadPin(GPIO_SW_GPIO_Port,GPIO_SW_Pin)){
		  is_clicked=2;
	  }
